...
public class SmartPhone {
	...
	// 요구사항
	// SmartPhone 클래스를 정의합니다. 이 클래스는 연락처 정보를 관리하는 클래스입니다.
	// Contact 클래스의 인스턴스 10개를 저장 할 수 있는 배열을 정의합시다.
	// => 10개 정보를 저장할 수 있다. -> 배열 인스턴스를 생성 Contact[]
	// Contact 클래스 기반의 인스턴스가 최대 10개 까지 만들어서 배열에 참조값을 저장
	...
	private static SmartPhone sp = new SmartPhone(maxSize);

	public static SmartPhone getInstance() {
		if (sp == null) {
			sp = new SmartPhone(maxSize);
		}
		return sp;
	}
	...
	void editContact() {
		...
		// 삭제하고자 하는 index 찾아야 한다 -> 왼쪽으로 시프트
		int searchIndex = -1;	// 현재 검색의 결과는 없다. 0으로 주면 못찾아도 유효한 배열 주소라 안됨
		...
		// 위험한 코드임. 하나하나 따로 바꾸다가 알 수 없는 예외 오류가 발생하면 저장부분을 초기화 시켜서 다시 입력 시켜야함. 예외처리
	...
	void deleteContact() {
		...
		// 검색한 index 값으로 분기 : 시프트를 하거나 검색 결과 이름 존재x
		if(searchIndex<0) {
